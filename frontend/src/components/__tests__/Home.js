...
describe('create Home', () => {
  it('renders', () => {
    ...
    const sourceInput = wrapper.find('Input').at(0)
    ...
    const minDfC = wrapper.find('Input').at(3)
    const maxDfC = wrapper.find('Input').at(4)
    expect(minDfC.prop('disabled')).toBe(true)
    expect(maxDfC.prop('disabled')).toBe(true)
    // test threshold change
    const thresholdC = wrapper.find('Input').at(2)
